...
# Application sets AGENT_LOGS_DIR / AGENT_PROMPTS_DIR before importing.
# Defaults assume cwd is the application root with logs/ and prompts/ subdirs.
LOGS_DIR = Path(os.environ.get("AGENT_LOGS_DIR", Path.cwd() / "logs")).resolve()
PROMPTS_DIR = Path(os.environ.get("AGENT_PROMPTS_DIR", Path.cwd() / "prompts")).resolve()
MEMORY_ROOT = LOGS_DIR / "memory"
